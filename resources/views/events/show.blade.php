    ...
    <div class="col-md-10 offset-md-1">
        <div class="row">
            ...
            <div id="info-container" class="col-md-6">
                ...
                @if($user->id !== $eventOwner['id'])
                    @if($hasUserJoined)
                        <form action="/events/leave/{{ $event->id }}" method="POST" >
                            @csrf
                            @method('DELETE')
                            <a href="/events/leave/{{ $event->id }}" class="btn btn-danger"  id="event-submit" onclick="event.preventDefault(); this.closest('form').submit(); " >Remover Presença</a>
                        </form>
                    @else
                    ...
                    @endif
                @else
                    <div class="is-event-owner" >Esse evento é seu</div>
                @endif
